...
typedef struct {
	...
	data_block_number direct[NUM_DIRECT_INODES], /* data_blocks */
			  indirect;		/* points to a singly indirect block */
} inode;
...
- `indirect` is the offset number (`data_block_number`) of a data block, which contains `NUM_DIRECT_INODES` number of `data_block_number`'s.
